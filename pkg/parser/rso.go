...
func (p *Parser) GenerateAssets() error {
	// Get Plan
	err := p.getPlan()
	if err != nil {
	...
	err = p.GenerateResourceOverview()
	if err != nil {
		return err
	}
...
func (p *Parser) PopulateModuleLocations(moduleJSONFile string, locations map[string]string) {
...
func (p *Parser) PopulateConfigs(parent string, parentKey string, rso *ResourcesOverview, config *tfjson.ConfigModule) {
	ml := rso.Locations
	rc := rso.Configs
	...
	for moduleName, m := range config.ModuleCalls {
		...
		p.PopulateConfigs(mn, childKey, rso, m.Module)
	}
}

func (p *Parser) PopulateModuleState(rso *ResourcesOverview, module *tfjson.StateModule, prior bool) {
...
func (p *Parser) GenerateResourceOverview() error {
	...
	moduleJSONPath := filepath.Join(p.WorkingDir, ".terraform/modules/modules.json")
	p.PopulateModuleLocations(moduleJSONPath, rso.Locations)

	// Create root module configuration
	rc[""] = &ConfigOverview{}
	rootModule, _ := tfconfig.LoadModule(p.WorkingDir)
	// If module can be loaded from filesystem
	...
	rc[""].ModuleConfig.Module = p.Plan.Config.RootModule

	p.PopulateConfigs("", "", rso, p.Plan.Config.RootModule)

	// Populate prior state
	if p.Plan.PriorState != nil {
		if p.Plan.PriorState.Values != nil {
			if p.Plan.PriorState.Values.RootModule != nil {
				p.PopulateModuleState(rso, p.Plan.PriorState.Values.RootModule, true)
			}
		}
	}

	// Populate planned state
	if p.Plan.PlannedValues != nil {
		if p.Plan.PlannedValues.RootModule != nil {
			p.PopulateModuleState(rso, p.Plan.PlannedValues.RootModule, false)
		}
	}
	...
	for outputName, output := range p.Plan.OutputChanges {
	...
	for _, resource := range p.Plan.ResourceChanges {
		id := resource.Address
