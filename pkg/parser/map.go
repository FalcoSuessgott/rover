...
func (p *Parser) GenerateModuleMap(parent *Resource, parentModule string) {
	childIndex := regexp.MustCompile(`\[[^[\]]*\]$`)
	matchBrackets := regexp.MustCompile(`\[[^\[\]]*\]`)

	states := p.RSO.States
	configs := p.RSO.Configs
	...
	if parentConfigured && !states[parentModule].IsParent {
		for oName, o := range configs[parentConfig].Module.Outputs {
			...
			p.AddFileIfNotExists(parent, parentModule, fname)
		...
		if rs.Type == ResourceTypeResource || rs.Type == ResourceTypeData {
			...
			if configured {
				...
				if rs.Type == ResourceTypeData && configs[parentConfig].Module.DataResources[ind] != nil {
					...
					p.AddFileIfNotExists(parent, parentModule, DefaultFileName)
...
func (p *Parser) AddFileIfNotExists(module *Resource, parentModule string, fname string) {
...
// Defaults to config
func (p *Parser) GenerateMap() error {
	...
	rootConfig := p.RSO.Configs[""].Module
	...
	if rootConfig != nil {
		...
		p.GenerateModuleMap(rootModule, "")
	} else {
		p.AddFileIfNotExists(rootModule, "", DefaultFileName)
		p.GenerateModuleMap(rootModule.Children[DefaultFileName], "")
	}
