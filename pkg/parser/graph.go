...
// GenerateGraph -
func (p *Parser) GenerateGraph() error {
	log.Println("Generating resource graph...")

	nodes := p.GenerateNodes()
	edges := p.GenerateEdges()
		...
		if strings.Contains(e.Data.ID, "terraform.workspace") {
			nodes = append(nodes, Node{
				Data: NodeData{
					...
					Parent: strings.ReplaceAll(p.Map.Path, "./", ""),
				},
				Classes: "locals",
			})
			break
...
func (p *Parser) addNodes(base string, parent string, nodeMap map[string]Node, resources map[string]*Resource) []string {
	nmo := []string{}
	...
	for id, re := range resources {
		if re.Type == ResourceTypeResource || re.Type == ResourceTypeData {
			...
			nmo = append(nmo, p.addNodes(base, id, nodeMap, re.Children)...)
			...
			nmo = append(nmo, p.addNodes(base, fid, nodeMap, re.Children)...)
		} else {
...
// GenerateNodes -
func (p *Parser) GenerateNodes() []Node {
	nodeMap := make(map[string]Node)
	nmo := []string{}

	basePath := strings.ReplaceAll(p.Map.Path, "./", "")
	...
	nmo = append(nmo, p.addNodes(basePath, basePath, nodeMap, p.Map.Root)...)
...
func (p *Parser) addEdges(base string, parent string, edgeMap map[string]Edge, resources map[string]*Resource) []string {
	emo := []string{}
	for id, re := range resources {
		...
		if p.RSO.Configs[configId] != nil {
			// If Resource
			if p.RSO.Configs[configId].ResourceConfig != nil {
				expressions = p.RSO.Configs[configId].ResourceConfig.Expressions
				// If Module
			} else if p.RSO.Configs[configId].ModuleConfig != nil {
				expressions = p.RSO.Configs[configId].ModuleConfig.Expressions
				// If Output
			} else if p.RSO.Configs[configId].OutputConfig != nil {
				expressions = make(map[string]*tfjson.Expression)
				expressions["output"] = p.RSO.Configs[configId].OutputConfig.Expression
			}
		}
		// fmt.Printf("%+v - %+v\n", oName, oValue)
		...
		if re.Type == ResourceTypeFile {
			emo = append(emo, p.addEdges(base, parent, edgeMap, re.Children)...)
		} else {
			emo = append(emo, p.addEdges(base, id, edgeMap, re.Children)...)
		}
	}
...
// GenerateEdges -
func (p *Parser) GenerateEdges() []Edge {
	edgeMap := make(map[string]Edge)
	emo := []string{}

	// config := r.Plan.Config.RootModule

	emo = append(emo, p.addEdges("", "", edgeMap, p.Map.Root)...)
